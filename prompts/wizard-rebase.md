...
3. **Per repo that needs rebasing:**
   ...
   f. **Verify nothing's broken.** Run the repo's relevant test suite per `/wizard` skill Phase 5 — TDD matters here, the rebase may have silently broken your code even without conflict markers. If tests fail, do NOT force-push; try to fix the failure inline (it's usually a symbol rename or import change from upstream). If fixable, commit the fix with message `rebase: adapt to upstream <short description>` and `--amend` it into the rebased top commit (`git commit --amend --no-edit` after adding the fix to the final commit, or as a new commit if the fix is substantive). If unfixable in this session: `git rebase --abort` if still in progress OR reset to the pre-rebase state, then `REBASE_FAILED: tests failed post-rebase in <repo> — <one-line reason>`, exit non-zero.
