...
5. **Process architect outputs.** For every architect run whose `plan.json` is written and heartbeat absent: transition the architect to `awaiting-architect-review` and spawn an **architect-review wizard** that reads `plan.json` + `design.md`, fixes defects directly (rewrites mandates, removes spurious deps, adjusts boundaries — like a senior reviewer with push access), and returns `approve` or `reject`. Approve advances the architect to `awaiting-tier-2`; reject escalates to the user with the reviewer's `concerns_unfixed`. **There is no architect refer-back cycle.** See `prompts/wizard-architect-review.md`.
6. **Spawn designer wizard sessions** for every architect at `awaiting-tier-2` whose cross-epic deps are satisfied. A sub-epic with `depends_on: [X, Y]` waits until every issue in sub-epic X's and Y's manifests has merged (or been archived). Strict gate: no parallel design across dependent sub-epics. Sub-epics with no deps spawn as concurrency allows. Each designer's `manifest.json` then triggers a **design-review wizard** symmetric to the architect-review: reads the manifest + Linear epic + issues, fixes defects via Edit/Write on `manifest.json` and `mcp__plugin_linear_linear__save_issue` on Linear records, returns approve/reject. Approve advances the designer to `awaiting-tier-3`; reject escalates. See `prompts/wizard-design-review.md`.
