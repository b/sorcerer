...
|---|---|---|
| Wizard session crash | Tick-step-5 check: `kill -0 <pid>` fails. The pid liveness check runs before heartbeat staleness so crashes are caught within one tick (~30s), not after the 5-minute heartbeat window. Classification: OK marker → awaiting-review; FAILED marker → escalate; 429 → throttled; no marker → respawn once, then escalate with `rule: <mode>-no-output`. | Respawn once (respawn_count++); second failure → escalate with a PushNotification on the escalations.log append. |
| Wizard stuck (no crash, but heartbeat stops advancing) | Tick step 11: heartbeat mtime > 5 minutes | Respawn once; second failure → escalate |
